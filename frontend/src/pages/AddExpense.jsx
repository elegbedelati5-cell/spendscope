...
const inputBase =
  'mt-1.5 w-full min-h-11 rounded-xl border bg-white px-3.5 py-2.5 text-base text-slate-900 shadow-sm transition-colors placeholder:text-slate-400 focus:outline-none focus:ring-2 sm:text-sm'
const inputNormal = `${inputBase} border-slate-200 focus:border-indigo-500 focus:ring-indigo-500/25`
...
export default function AddExpense() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (addAnother) {
        ...
        navigate('/dashboard', { replace: true, state: { flash: 'Expense saved successfully.' } })
      }
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-xl px-0 sm:px-1">
      ...
      {savedBanner ? (
        <div
          className="mb-6 flex items-start justify-between gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900 shadow-sm"
          role="status"
        >
      ...
      <form
        ...
        className="space-y-8 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-md ring-1 ring-slate-100 sm:p-8"
        noValidate
      >
        {serverError ? (
        ...
        <section className="space-y-3">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500">Enter the total in Nigerian naira (NGN).</p>
          </div>
          <input
            id="amount"
            type="number"
            ...
            <div className="flex flex-wrap gap-2">
              {QUICK_AMOUNTS.map((v) => (
                <button
                  ...
                  className="min-h-10 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-700 transition-colors hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-800 active:scale-[0.98] sm:text-sm"
                >
                  {formatNaira(v)}
                </button>
              ))}
        ...
        <section className="border-t border-slate-100 pt-8">
          <div className="mb-3">
            ...
            <p className="mt-0.5 text-xs text-slate-500">What kind of spending was this?</p>
          </div>
          <div
        ...
        <section className="space-y-3 border-t border-slate-100 pt-8">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500">Short note so you remember later (store, item, etc.).</p>
          </div>
          <input
            id="description"
            type="text"
        ...
        <div className="border-t border-slate-100 pt-6">
          <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-100 bg-slate-50/50 px-4 py-3">
            ...
            <span className="text-sm text-slate-700">
              <span className="font-medium text-slate-900">Add another expense after saving</span>
        ...
        <div className="flex flex-col gap-3 pt-2 sm:flex-row-reverse sm:justify-start">
          ...
          <button
            ...
            disabled={loading}
            onClick={() => navigate(-1)}
            className="inline-flex min-h-12 w-full items-center justify-center rounded-xl border border-slate-200 bg-white px-5 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50 sm:w-auto"
          >
            Cancel
          </button>
        </div>
