  ...
  return (
    <div className="mx-auto max-w-2xl space-y-10 pb-8">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-xs text-slate-500">Your name appears in the sidebar and dashboard greeting.</p>
        <form onSubmit={saveProfile} className="mt-5 space-y-4">
          {profileErr ? (
            <p className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700">{profileErr}</p>
          ) : null}
          {profileMsg ? (
            <p className="rounded-lg bg-emerald-50 px-3 py-2 text-sm text-emerald-800">{profileMsg}</p>
          ) : null}
          <div>
            ...
            <input
              ...
              className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/25"
            />
          </div>
          <div>
            ...
            <input
              ...
              disabled
              value={user?.email ?? ''}
              className="mt-1 w-full cursor-not-allowed rounded-xl border border-slate-100 bg-slate-50 px-3 py-2 text-sm text-slate-500"
            />
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <form onSubmit={submitPassword} className="mt-5 space-y-4">
          {pwErr ? <p className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700">{pwErr}</p> : null}
          {pwMsg ? (
            <p className="rounded-lg bg-emerald-50 px-3 py-2 text-sm text-emerald-800">{pwMsg}</p>
          ) : null}
          <div>
            ...
            <input
              ...
              className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/25"
            />
          </div>
          <div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {currencyMsg ? <p className="mt-3 text-sm text-emerald-700">{currencyMsg}</p> : null}
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-5 space-y-4">
          <label className="flex cursor-pointer items-center justify-between gap-3 rounded-xl border border-slate-100 bg-slate-50/80 px-4 py-3">
            <span className="text-sm text-slate-700">Expense reminders</span>
            <input
              type="checkbox"
          ...
          <label className="flex cursor-pointer items-center justify-between gap-3 rounded-xl border border-slate-100 bg-slate-50/80 px-4 py-3">
            <span className="text-sm text-slate-700">Monthly spending summaries</span>
            <input
              type="checkbox"
          ...
          {notifMsg ? <p className="text-sm text-emerald-700">{notifMsg}</p> : null}
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <Link
          ...
          className="mt-4 inline-flex rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-indigo-600 hover:bg-slate-50"
        >
          Go to transactions
        </Link>
      </section>
      ...
      <section className="rounded-2xl border border-red-200 bg-red-50/50 p-6 shadow-sm">
        ...
        {!deleteOpen ? (
          ...
          <div className="mt-4 space-y-3">
            {deleteErr ? <p className="text-sm text-red-700">{deleteErr}</p> : null}
            <div>
              <label htmlFor="delete-pw" className="block text-sm font-medium text-red-900">
              ...
              <input
                ...
                className="mt-1 w-full max-w-sm rounded-xl border border-red-200 bg-white px-3 py-2 text-sm"
              />
            </div>
